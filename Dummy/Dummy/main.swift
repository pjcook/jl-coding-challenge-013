...
func parse(_ rawData: String) -> [Place] {
    var places = [Place]()
    let rawData = rawData.replacingOccurrences(of: ",,,", with: ",na,na,")
    let components = rawData.split(separator: ",")
    let stepSize = 4
    var position = 0
    
    while position <= components.count-stepSize {
        let name = String(components[position])
        let postcode = String(components[position+1])
        if let lat = Double(components[position+2]), let lng = Double(components[position+3]) {
            let place = Place(name: name, postcode: postcode, location: CLLocation(latitude: lat, longitude: lng))
            places.append(place)
        }
...
func nearestLocation(_ currentLocation: CLLocation, places: [Place]) -> Place? {
    guard var place = places.first else { return nil }
    var minDistance = place.location.distance(from: currentLocation)
    places.forEach {
        let distance = $0.location.distance(from: currentLocation)
...
func remove(_ place: Place, from places: [Place]) -> [Place] {
    var places = places
    if let index = places.firstIndex(where: { $0.location == place.location }) {
        places.remove(at: index)
    }
    return places
}

var places = parse(data)
...
places = remove(headOffice, from: places)

while places.count > 0 {
    guard let nearest = nearestLocation(currentLocation, places: places) else { break }
